  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-[#09090b] text-zinc-100">
      <div className="w-full max-w-sm p-8 bg-zinc-900 rounded-2xl border border-zinc-800 shadow-xl">
        <h2 className="text-2xl font-semibold mb-6 text-center text-zinc-100">Create account</h2>
        {error && <div className="mb-4 p-3 text-sm text-red-400 bg-red-950/50 rounded-lg">{error}</div>}
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium mb-1.5 text-zinc-400">Name</label>
            <input
              type="text"
              value={name}
        ...
        <p className="mt-6 text-center text-sm text-zinc-400">
          Already have an account? <Link to="/login" className="text-zinc-100 hover:underline">Login</Link>
        </p>
      </div>
    </div>
  );
